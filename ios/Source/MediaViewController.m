...
static NSTimeInterval const MediaViewControllerAnimationDuration = 0.75f;
static NSTimeInterval const MediaViewControllerProgressBarHeight = 2.00f;
static NSString *MediaViewControllerSymLinkedMediaTempPath = @"com.layer.atlas/media/";

@interface MediaViewController () <UIScrollViewDelegate, LYRProgressDelegate>
...
- (void) viewDidLoad
{
    ...
    if (ATLMessagePartForMIMEType(self.message, ATLMIMETypeImageJPEG) || ATLMessagePartForMIMEType(self.message, ATLMIMETypeImagePNG) || ATLMessagePartForMIMEType(self.message, ATLMIMETypeImageGIF)) {
        ...
        self.mediaBaseURL = [symLinkedMediaBaseURL URLByAppendingPathComponent: MediaViewControllerSymLinkedMediaTempPath].absoluteURL;
...
- (void) viewWillAppear: (BOOL) animated
{
    ...
    if (self.viewControllerConfigured) return;
...
- (void) viewWillLayoutSubviews
{
    ...
    self.progressView.frame = CGRectMake(0,
                                         self.navigationController.navigationBar.frame.size.height - MediaViewControllerProgressBarHeight,
                                         self.view.frame.size.width,
                                         MediaViewControllerProgressBarHeight);
}
...
{
    if (self.zoomingEnabled) [self configureForAvailableSpace];
}
...
{
    if (self.scrollView.minimumZoomScale == self.scrollView.maximumZoomScale || !self.zoomingEnabled) return;

    if (self.scrollView.zoomScale == self.scrollView.minimumZoomScale) {
        CGPoint tappedPoint = [gestureRecognizer locationInView: self.lowResImageView];
...
{
    // Removing the full-resolution image to save GPU resources when animating the view controller POP.
...
- (void) loadFullResImage
{
    ...
    if (!self.fullResImage) return;

    self.fullResImageView.frame = self.mediaViewFrame;

    [UIView animateWithDuration:MediaViewControllerAnimationDuration animations:^{
...
- (void) loadFullResVideo
{
    ...
    if (!(fullResVideoPart.transferStatus == LYRContentTransferReadyForDownload || fullResVideoPart.transferStatus == LYRContentTransferDownloading)) {
        if (self.moviePlayerController) return;
        ...
        [[NSNotificationCenter defaultCenter] addObserver: self
                                              selector:    @selector(moviePlayerWillChangeFullScreenAppearance:)
...
ofObject:               (LYRMessagePart *) messagePart
