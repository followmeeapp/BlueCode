...
int main(int argc, char** argv) {
  ...
  if (VarMap.count("in-edge") && VarMap.count("in-vertex")) {
    ...
    // all input streams are set, lets proceed!
    ErrorPtr e;

    auto Graph = lmdb_graph<std::string, double>(e, DBPath, 9192);
    if (e)
      std::cout << e.get()->message() << std::endl;
    ON_ERROR_RETURN_VAL(e, 1);
    ERROR_RESET(e);

    Transaction txn = Graph.beginTransaction(e);
    ON_ERROR_RETURN_VAL(e, 1);
    ERROR_RESET(e);
    ...
    for (string Line; getline(VertexFile, Line);) {
      ...
      Graph.addVertex(e, VertexID, to_string(VertexID), txn);
    ...
    cout << "in: " << chrono::duration<double, milli>(LoadDuration).count()
         << " ms\n";
    ERROR_RESET(e);

    LoadStartTime = chrono::steady_clock::now();

    std::map<uint64_t, std::vector<uint64_t>> InEdgesMap;
    std::map<uint64_t, std::vector<uint64_t>> OutEdgesMap;

    ItemCounter = 0;
    txn = Graph.beginTransaction(e);
    if (e) cout << "failed: " << e.get()->message();
    ON_ERROR_RETURN_VAL(e, 1);
    ...
    if (e) cout << "failed: " << e.get()->message();
    ON_ERROR_RETURN_VAL(e, 1);
    LoadEndTime = chrono::steady_clock::now();
