...
engine::engine(ErrorPtr &e, LMDBEnv &env)
...
void engine::beginTransaction(ErrorPtr &e, TransBody body) {
...
void engine::commitTransaction(ErrorPtr &e, Transaction &txn) {
  ASSERT_ERROR_RESET(e);
  env_.commitTransaction(e, txn);
}

void engine::handleDiscoverRequest(ErrorPtr &e, int64_t deviceID,
                                   DiscoverRequest::Reader request,
                                   Transaction &txn) {
  ASSERT_ERROR_RESET(e);
  ...
  auto Session = make_reader(*SessionVal).getRoot<DiscoverRequest>();
  ...
  bool needDeviceUpdate = RequestSyncInfo.getLastDeviceUpdate() >
                          SessionSyncInfo.getLastDeviceUpdate();
  bool needLatestRecentCard =
      RequestSyncInfo.getLastCard() > SessionSyncInfo.getLastCard();
  ...
  if (CachedResponseVal != nullptr) {
    auto CachedResponse =
        make_reader(*CachedResponseVal).getRoot<DiscoverResponse>();
  ...
  if (needCardsSync || needDeviceUpdate || needLatestRecentCard) {
    ...
    auto SRequest = builder.initRoot<SyncRequest>();
    SRequest.setDiscoverInfo(request.getDiscoverInfo());
    SRequest.setLatestRecentCard(SessionSyncInfo.getLastCard());
    SRequest.setWantsCardsAdvertisedByDevice(needCardsSync);
    SRequest.setLatestDiscoveredDevice(SessionSyncInfo.getLastDeviceUpdate());
...
void engine::handleSyncResponse(ErrorPtr &e, int64_t deviceID,
                                SyncResponse::Reader response,
                                Transaction &txn) {
  ASSERT_ERROR_RESET(e);
  ...
  for (auto device : Devices) {
    ...
    auto Record = DBVal(sizeof(info_record), static_cast<void *>(&DevRec));
  ...
  for (auto card : RecentCards) {
    ...
    auto Record = DBVal(sizeof(info_record), static_cast<void *>(&CardRec));
...
void engine::handleDiscoverCards(ErrorPtr &e, int64_t deviceID,
  ...
  for (auto discDevice : DiscoverdDevices) {
    ...
    while (hasVal(Card)) {
      auto Record =
          DBVal(sizeof(info_record), static_cast<void *>(&Card->value));
...
void engine::handleDeactivateDevice(ErrorPtr &e, int64_t deviceID,
                                    DiscoverInfo::Reader discInfo,
                                    SyncInfo::Reader syncInfo,
                                    Transaction &txn) {
  ASSERT_ERROR_RESET(e);
  ...
  if (hasVal(CardBuffer)) {
    ...
    TimeStampSection.setLastCard(syncInfo.getLastCard());
  }

  sendDiscoverResponse(e, deviceID, Response);
}

void engine::handleDiscoverAckResponse(ErrorPtr &e, int64_t deviceID,
                                       DiscoverAckResponse::Reader response,
                                       Transaction &txn) {
  ASSERT_ERROR_RESET(e);
...
void engine::processCachedResponse(ErrorPtr &e, int64_t deviceID,
                                   int64_t timeStamp, Transaction &txn) {
  ASSERT_ERROR_RESET(e);
  LOG("we're processing the AckResponse!\n")
  ...
  // we have a response, let's process it
  auto Response = make_reader(*ResponseVal).getRoot<DiscoverResponse>();
...
void engine::processAckResponseSection(ErrorPtr &e, int64_t deviceID,
                                       DiscoverResponse::Reader response,
                                       TimestampSection::Reader section,
                                       Transaction &txn) {
  ASSERT_ERROR_RESET(e);
  LOG("we're processing the Section!\n")
  ...
  auto Record = DBVal(sizeof(info_record), (void *)&CardRec);
  ...
  for (auto recentCard : RemoveCards) {
    auto Record = DBVal(sizeof(info_record), (void *)&recentCard);
  ...
  auto DeviceRecord = DBVal(sizeof(info_record), (void *)&DeviceRec);
  ...
  for (auto device : RemoveDevices) {
    auto Record = DBVal(sizeof(info_record), (void *)&device);
...
void engine::sendSyncRequest(ErrorPtr &e, int64_t deviceID,
...
void engine::sendDiscoverResponse(ErrorPtr &e, int64_t deviceID,
                                  DiscoverResponse::Reader response) {
  ASSERT_ERROR_RESET(e);
